...
class TagBasedPermissionManager(AirflowSecurityManager):
    ...
    def get_user_permissions(self, user):
        ...
        # Retrieve the user's role from the security manager
        ...
        for dag_id, dag in all_dags.items():
            ...
            if "b2b_viewer" in user_roles:
                allowed_tags = {"b2b"}  # Tags allowed for this role
            elif "b2c_viewer" in user_roles:
                allowed_tags = {"b2c"}  # Tags allowed for this role
            elif any(role in user_roles for role in ["admin", "ops", "viewer"]):
            ...
            if dag_tags & allowed_tags:
                user_permissions[dag_id] = {"can_read": True, "can_edit": "viewer" not in user_roles}
            else:
